...
public class Experimenter {
	...
	public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations,
			int max_cycles, int max_inactive_cycles, boolean visualize) throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, System.out, -1, false);
	}

	public static void runExperimentsPartiallyObservable(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt,
			int iterations, int max_cycles, int max_inactive_cycles, boolean visualize) throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, System.out, -1, true);
	}

	public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations,
			int max_cycles, int max_inactive_cycles, boolean visualize, PrintStream out) throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, out, -1, false);
	}

	public static void runExperimentsPartiallyObservable(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt,
			int iterations, int max_cycles, int max_inactive_cycles, boolean visualize, PrintStream out)
			throws Exception {
	...
	public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations,
			...
			throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, out, null,
	...
	public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations,
			int max_cycles, int max_inactive_cycles, boolean visualize, PrintStream out,
			int run_only_those_involving_this_AI, boolean skip_self_play, boolean partiallyObservable,
			boolean saveTrace, String traceDir) throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, out, null,
				run_only_those_involving_this_AI, skip_self_play, partiallyObservable, saveTrace, false, traceDir);
	}

	public static void runExperiments(List<AI> bots, List<PhysicalGameState> maps, UnitTypeTable utt, int iterations,
			int max_cycles, int max_inactive_cycles, boolean visualize, PrintStream out1, PrintStream out2,
			int run_only_those_involving_this_AI, boolean skip_self_play, boolean partiallyObservable,
			boolean saveTrace, String traceDir) throws Exception {
		runExperiments(bots, maps, utt, iterations, max_cycles, max_inactive_cycles, visualize, out1, out2,
				run_only_those_involving_this_AI, skip_self_play, partiallyObservable, saveTrace, false, traceDir);
	}
...
//over several games
						...
						if (saveTrace) {
							...
							if (saveZip) {
								zip = new ZipOutputStream(
										new FileOutputStream("replays_micro_rts" + filename + ".zip"));
		...
		/*
		 * System.out.println("RESULT TOP"); System.out.println("Wins: "); for (int
		 * ai1_idx = 0; ai1_idx < bots.size(); ++ai1_idx) { for (int ai2_idx = 0;
		 * ai2_idx < bots.size(); ++ai2_idx) System.out.print(top_wins[ai1_idx][ai2_idx]
		 * + ", "); System.out.println(""); }
		 * 
		 * System.out.println("Ties: "); for (int ai1_idx = 0; ai1_idx < bots.size();
		 * ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size(); ++ai2_idx)
		 * System.out.print(top_ties[ai1_idx][ai2_idx] + ", "); System.out.println("");
		 * }
		 * 
		 * System.out.println("Loses: "); for (int ai1_idx = 0; ai1_idx < bots.size();
		 * ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size(); ++ai2_idx)
		 * System.out.print(top_loses[ai1_idx][ai2_idx] + ", "); System.out.println("");
		 * }
		 * 
		 * System.out.println("Win average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (top_wins[ai1_idx][ai2_idx] > 0)
		 * System.out.print((top_win_time[ai1_idx][ai2_idx] /
		 * top_wins[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Tie average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (top_ties[ai1_idx][ai2_idx] > 0)
		 * System.out.print((top_tie_time[ai1_idx][ai2_idx] /
		 * top_ties[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Lose average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (top_loses[ai1_idx][ai2_idx] > 0)
		 * System.out.print((top_lose_time[ai1_idx][ai2_idx] /
		 * top_loses[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("#####\nRESULT BOTTOM"); System.out.println("Wins: "); for
		 * (int ai1_idx = 0; ai1_idx < bots.size(); ++ai1_idx) { for (int ai2_idx = 0;
		 * ai2_idx < bots.size(); ++ai2_idx)
		 ...
		 * System.out.println("Win average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (bottom_wins[ai1_idx][ai2_idx] > 0)
		 * System.out.print((bottom_win_time[ai1_idx][ai2_idx] /
		 * bottom_wins[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Tie average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (bottom_ties[ai1_idx][ai2_idx] > 0)
		 * System.out.print((bottom_tie_time[ai1_idx][ai2_idx] /
		 * bottom_ties[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Lose average time: "); for (int ai1_idx = 0; ai1_idx <
		 * bots.size(); ++ai1_idx) { for (int ai2_idx = 0; ai2_idx < bots.size();
		 * ++ai2_idx) if (bottom_loses[ai1_idx][ai2_idx] > 0)
		 * System.out.print((bottom_lose_time[ai1_idx][ai2_idx] /
		 * bottom_loses[ai1_idx][ai2_idx]) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("#####\nGLOBAL RESULTS"); System.out.println("Wins: ");
		 * for (int ai1_idx = 0; ai1_idx < bots.size(); ++ai1_idx) { for (int ai2_idx =
		 * 0; ai2_idx < bots.size(); ++ai2_idx)
		 ...
		 * ++ai2_idx) if (bottom_wins[ai1_idx][ai2_idx] > 0)
		 * System.out.print(((bottom_win_time[ai1_idx][ai2_idx] +
		 * top_win_time[ai1_idx][ai2_idx]) / (bottom_wins[ai1_idx][ai2_idx] +
		 * top_wins[ai1_idx][ai2_idx])) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Tie average time: "); for (int ai1_idx = 0; ai1_idx <
		 ...
		 * ++ai2_idx) if (bottom_ties[ai1_idx][ai2_idx] > 0)
		 * System.out.print(((bottom_tie_time[ai1_idx][ai2_idx] +
		 * top_tie_time[ai1_idx][ai2_idx]) / (bottom_ties[ai1_idx][ai2_idx] +
		 * top_ties[ai1_idx][ai2_idx])) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 * 
		 * System.out.println("Lose average time: "); for (int ai1_idx = 0; ai1_idx <
		 ...
		 * ++ai2_idx) if (bottom_loses[ai1_idx][ai2_idx] > 0)
		 * System.out.print(((bottom_lose_time[ai1_idx][ai2_idx] +
		 * top_lose_time[ai1_idx][ai2_idx]) / (bottom_loses[ai1_idx][ai2_idx] +
		 * top_loses[ai1_idx][ai2_idx])) + ", "); else System.out.print("-, ");
		 * System.out.println(""); }
		 */
	}
